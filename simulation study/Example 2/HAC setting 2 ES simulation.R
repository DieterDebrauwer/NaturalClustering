...
#In this script we consider Expected shortfall for Setting 2 from Table 6.
...
#save(weightEstimateMatrix, file="simulationStudyHACES23SampleSize400Rdata")
...
weightEstimateMatrixOpt2 <- foreach(j = 1:500, .combine = rbind, .packages = c("HAC", "copula", "stats")) %dopar% {
  ...
  ExtrOpt2 <- apply_ES(0.95, weight_matrix, GiantSampleMatrix)
...
save(weightEstimateMatrixOpt1,weightEstimateMatrixOpt2,weightEstimateMatrixOpt3Representative,weightEstimateMatrixOpt4,weightEstimateMatrixOpt5,file="simulationStudyHACES23SampleSize400Opt1To5.Rdata")
