...
#In this script we consider extremile for Setting 2 from Table 6.
...
## Function: apply_Extr
# Purpose: Estimate extremile at level tau for a whole grid of portfolio weights.
# Input:
...
# Output: vector of extremile estimates (one per row of weight_matrix).
...
#save(weightEstimateMatrix, file="simulationStudyHACextremile23SampleSize400.Rdata")
...
weightEstimateMatrixOpt1 <- foreach(j = 1:500, .combine = rbind, .packages = c("HAC", "copula", "stats")) %dopar% {
  ...
  c(c(cluster1*Opt1Between[1],cluster2*Opt1Between[2]),ExtrOpt1)
}
...
weightEstimateMatrixOpt2 <- foreach(j = 1:500, .combine = rbind, .packages = c("HAC", "copula", "stats")) %dopar% {
  ...
  ExtrOpt2 <- apply_Extr(0.95, weight_matrix, GiantSampleMatrix)
  c(c(cluster1*Opt1Between[1],cluster2*Opt1Between[2]),ExtrOpt2)
}
...
###############################################
###############################################
##Optimization 3 with argmin
###############################################
###############################################
#Estimate extremile parametrically for exponential data
ExtrMarginsExp=function(data){
  tau=0.95
  d_tau <- function(x, tau) {
    (log(0.5) / log(tau) * x^(log(0.5) / log(tau) - 1) * as.numeric(tau > 0.5) * as.numeric(tau <= 1)) * as.numeric(x <= 1)
  }
...
#Estimate extremile parametrically for normal data
ExtrMarginsNorm=function(data){
  tau=0.95
  d_tau <- function(x, tau) {
    (log(0.5) / log(tau) * x^(log(0.5) / log(tau) - 1) * as.numeric(tau > 0.5) * as.numeric(tau <= 1)) * as.numeric(x <= 1)
  }
...
weightEstimateMatrixOpt3Representative <- foreach(j = 1:500, .combine = rbind, .packages = c("HAC", "copula", "stats")) %dopar% {
  ...
  toMinimize1=c(ExtrMarginsExp(sample[,1]),ExtrMarginsNorm(sample[,2]))
  for (i in 1:2){
    sum=0
    for (k in 3:4){
  ...
  toMinimize2=c(ExtrMarginsNorm(sample[,3]),ExtrMarginsNorm(sample[,4]))
  for (i in 3:4){
    sum=0
    for (k in 1:2){
      sum=sum+abs(cor(sample[,i],sample[,k],method="kendall"))
...
weightEstimateMatrixOpt5 <- foreach(j = 1:500, .combine = rbind, .packages = c("HAC", "copula", "stats")) %dopar% {
  ...
  ExtrOpt5 <- apply_Extr(0.95, weight_matrix, GiantSampleMatrix)
  c(c(cluster1*1/2,cluster2*1/2),ExtrOpt5)}




save(weightEstimateMatrixOpt1,weightEstimateMatrixOpt2,weightEstimateMatrixOpt3Representative,weightEstimateMatrixOpt4,weightEstimateMatrixOpt5,file="simulationStudyHACextremile23SampleSize400Opt1To5.Rdata")
...
trueWeightSingle=c(0.01,0.38,0.30,0.31)

trueExtrOpt1=1.573776
trueWeightOpt1=c(0.0039,0.3861,0.3050,0.3050)

trueExtrOpt2=1.637306
trueWeightOpt2=c(0.0025,0.2475,0.3750,0.3750)

trueExtrOpt3Representative=1.619796
trueWeightOpt3Representative=c(0,0.41,0.59,0) 

trueExtrOpt4=1.831826
trueWeightOpt4=c(0.125,0.125,0.375,0.375)

trueExtrOpt5=1.616228
trueWeightOpt5=c(0.005,0.495,0.250,0.250) 
