...
#In this script we go over a coarse grid and optimize across each cluster (so stage 2)
...
combinations <- generate_combinations(numeric(0), rep(1, 8), 1)
...
coarseGrid[which.min(NPNPrisk),]#0.05 0.05 0.05 0.05 0.05 0.25 0.05 0.40 0.05
min(NPNPrisk)#37.89068
...
coarseGrid[which.min(NPNPrisk),]# 0.05 0.05 0.05 0.50 0.05 0.05 0.15 0.05 0.05
min(NPNPrisk)#92.00686
#Corresponding risk: (fill in weight from here into risk function from opt 1!!!!!!!)
risk(c(coarseGrid[which.min(NPNPrisk),]))#51.41539
...
Opt3Representative <- coarseGrid[which.min(riskPerWeight), ]#0.05 0.05 0.25 0.15 0.05 0.10 0.05 0.15 0.15
ExtrOpt3Representative=min(riskPerWeight)#52.00751
